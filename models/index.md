...
<div class="sidebar-layout">
  ...
  <div class="sidebar-content">
    ...
    <span class="kicker">Choose a method</span><h1>Families of models</h1>
    ...
    <details open>
      ...
      <p><strong>Julia implementation:</strong> <a href="/Siena.jl/dev/">Siena.jl</a> implements a subset of this framework. Inspect the fresh final simulation batch and convergence diagnostics. Selection based on evolving behavior and broad R effect parity remain incomplete; see <a href="/capabilities/">current coverage</a>.</p>
    </details>
    ...
    <details open>
      ...
      <p>History-dependent statistics capture reciprocity, repetition and other network effects alongside exogenous covariates. Ordinal models condition on an event and model which dyad acts; timing models also use waiting-time exposure. Ordinal estimation can compare the observed event with a full or sampled risk set. Duration fitting requires a timing likelihood.</p>
      <p>Event models apply to interaction sequences such as email exchanges and communication logs. The observation process determines whether only event order, elapsed times, or both can be analysed.</p>
      <p><strong>Julia implementations:</strong> <a href="/REM.jl/dev/">REM.jl</a> supports full or sampled ordinal risk sets; <a href="/Relevent.jl/dev/">Relevent.jl</a> provides full-risk-set ordinal likelihoods and supported exponential timing models. An ordinal sequence does not supply elapsed durations. Exact timing requires interval-constant statistics; continuous-decay hazard integration is unavailable.</p>
    </details>
    ...
    <details open>
      ...
      <p><strong>Implemented in:</strong> <a href="/ERGM.jl/dev/">ERGM.jl</a> and extensions (<a href="/TERGM.jl/dev/">TERGM.jl</a>, <a href="/ERGMCount.jl/dev/">ERGMCount.jl</a>, <a href="/ERGMEgo.jl/dev/">ERGMEgo.jl</a>, <a href="/ERGMMulti.jl/dev/">ERGMMulti.jl</a>, <a href="/ERGMRank.jl/dev/">ERGMRank.jl</a>, <a href="/ERGMUserterms.jl/dev/">ERGMUserterms.jl</a>)</p>
    <p>Implementation scope differs across these packages. Count models currently use MPLE; rank models also offer rank MCMC-MLE; TERGM supports conditional MPLE. A method family described here does not imply that every variant is implemented. Consult <a href="/capabilities/">estimator capabilities</a> before selecting a model.</p>
    </details>
